...
def main(N_CLIENTS, N_LEADERS, lr, l2_lambda, beta, select_rate, ROUNDS, seed):
    ...
    res_file = "exp__sync_hier__%s__%s__%s__%s__%s__%s__%s.txt" % (N_CLIENTS, N_LEADERS, lr, l2_lambda, beta, select_rate, seed)
    
    # Server
    test_data, testloader = server_prepare_data()
    server = Server(CF10Net, test_data, testloader, lr=lr, N_CLIENTS=N_CLIENTS, res_file=res_file, beta=beta, seed=seed, asynch=False)
    ...
    for i, data in enumerate(client_datas):
        ...
        client_list.append(Client(CF10Net, lambda x : torch.optim.SGD(x, lr=lr, momentum=0.9), data, id=i, l2_lambda=l2_lambda, seed=seed, asynch=False))
    
    # Leader
    leader_list = []
    for i in range(N_LEADERS):
        leader = Leader(CF10Net, i, asynch=False)
