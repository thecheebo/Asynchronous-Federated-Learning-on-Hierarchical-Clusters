...
def main(N_CLIENTS, lr, l2_lambda, beta, select_rate, ROUNDS, seed):
    ...
    res_file = "exp__sync_no_hier__%s__%s__%s__%s__%s__%s.txt" % (N_CLIENTS, lr, l2_lambda, beta, select_rate, seed)
    ...
    for i, data in enumerate(client_datas):
        client = Client(CF10Net, lambda x : torch.optim.SGD(x, lr=lr, momentum=0.9), data, id=i, l2_lambda=l2_lambda, seed=seed, asynch=False)
