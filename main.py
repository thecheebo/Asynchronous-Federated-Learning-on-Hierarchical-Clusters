...
def main(N_CLIENTS, N_LEADERS, TRAIN_ROUNDS, SELECT_CLIENT_FRAC):
    ...
    if N_LEADERS > 0:
        clients = [Client(CF10Net, lambda x : torch.optim.SGD(x, lr=0.001, momentum=0.9), dat, idnum=i, leader_id=int(i/int((N_CLIENTS/N_LEADERS))))
               for i, dat in enumerate(client_datas)]
        ...
        clients = [Client(CF10Net, lambda x : torch.optim.SGD(x, lr=0.001, momentum=0.9), dat, idnum=i)
               for i, dat in enumerate(client_datas)]
